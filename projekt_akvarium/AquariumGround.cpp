...
//Objekt spodku akvaria:
std::unique_ptr<ppgso::Mesh> AquariumGround::mesh;
std::unique_ptr<ppgso::Texture> AquariumGround::texture;
std::unique_ptr<ppgso::Shader> AquariumGround::shader;
...
AquariumGround::AquariumGround()
{
    ...
    if (!texture) texture = std::make_unique<ppgso::Texture>(ppgso::image::loadBMP("AquariumGround.bmp"));
    if (!mesh) mesh = std::make_unique<ppgso::Mesh>("AquariumGround.obj");
}
...
bool AquariumGround::update(Scene &scene, float delta_time)
{
...
void AquariumGround::render(Scene &scene)
{
    ...
    //Svetelne zlozky materialu
    shader->setUniform("MaterialDiffuse", glm::vec3(0.5f, 0.5f, 0.5f));
    shader->setUniform("MaterialSpecular", glm::vec3(0.5f, 0.5f, 0.5f));
